const { logEscalation } = require("./integrations/firebaseLogger");
...
const API_CONTRACT = {
  DEFAULT_REPLY:
    "I apologize, but I encountered a temporary issue. Please try again in a moment.",
  DEFAULT_ESCALATED: true,
  DEFAULT_CONFIDENCE: 0.0,
  FALLBACK_REPLY:
    "Our system is experiencing difficulties. A human agent will assist you shortly.",
...
function standardizeResponse(backendLogicResponse) {
  ...
  const reply =
    typeof backendLogicResponse.reply === "string" &&
    backendLogicResponse.reply.trim() !== ""
      ? backendLogicResponse.reply
      : API_CONTRACT.DEFAULT_REPLY;

  const escalated =
    typeof backendLogicResponse.escalated === "boolean"
      ? backendLogicResponse.escalated
      : API_CONTRACT.DEFAULT_ESCALATED;

  let confidence =
    typeof backendLogicResponse.confidence === "number"
...
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    if (safeResponse.escalated === true) {
      logEscalation({
        requestId,
        userMessage,
        botReply: safeResponse.reply,
